...
const FEEDBACK_CHANNEL = "pi-pr:feedback";
const FEEDBACK_PROTOCOL = 1;
const MESSAGE_TYPE = "pi-pr-review-feedback";
const FANCY_FOOTER_PROTOCOL = 1;
const FANCY_FOOTER_WIDGET_CHANNEL = "pi-fancy-footer:widget";
const FANCY_FOOTER_READY_CHANNEL = "pi-fancy-footer:ready";
const REVIEW_WIDGET_ID = "pi-pr.review-threads";
...
interface FancyFooterReadyEvent {
  protocol: typeof FANCY_FOOTER_PROTOCOL;
  version: string;
}
...
function isFancyFooterReadyEvent(
  value: unknown,
): value is FancyFooterReadyEvent {
  if (typeof value !== "object" || value === null) return false;
  const event = value as Partial<FancyFooterReadyEvent>;
  return (
    event.protocol === FANCY_FOOTER_PROTOCOL &&
...
export default async function (pi: ExtensionAPI) {
  ...
  let reviewWidget: ReviewWidgetStatus | undefined;
  let generation = 0;
  let sessionActive = false;
  let sessionCwd = process.cwd();
  let reviewWidgetTimer: ReturnType<typeof setTimeout> | undefined;

  const removeReviewWidget = (): void => {
  ...
  const publishReviewWidget = (): void => {
    if (!reviewWidget || (!watcher && reviewWidget.unresolvedThreadCount === 0)) {
      removeReviewWidget();
      return;
    }
    ...
    pi.events.emit(FANCY_FOOTER_WIDGET_CHANNEL, {
      ...
      widget: {
        ...
        description: "Shows unresolved review threads and active PR watching",
        content: {
          type: "text",
          text:
            reviewWidget.unresolvedThreadCount > 0
              ? `${reviewWidget.unresolvedThreadCount}`
              : "\u200b",
        },
        icon: {
          glyphs: watching
          ...
          color: watching ? "accent" : "text",
        },
        layout: { row: 1, position: 4, align: "left" },
      },
    });
  };
  ...
  const scheduleReviewWidgetRefresh = (): void => {
    if (!sessionActive) return;
    if (reviewWidgetTimer) clearTimeout(reviewWidgetTimer);
    reviewWidgetTimer = setTimeout(() => {
      void refreshReviewWidget();
    }, POLL_INTERVAL_MS);
  };

  const refreshReviewWidget = async (): Promise<void> => {
    if (!sessionActive) return;
    if (watcher) {
      scheduleReviewWidgetRefresh();
      return;
    }

    try {
      const target = await resolveCurrentPullRequest(pi, { cwd: sessionCwd });
      const snapshot = await fetchSnapshot(pi, sessionCwd, target);
      if (!sessionActive || watcher) return;
      if (!snapshot || (snapshot.state && snapshot.state !== "OPEN")) {
        reviewWidget = undefined;
      } else {
        reviewWidget = {
          target,
  ...
  const poll = async (expectedGeneration: number): Promise<void> => {
    ...
    current.unresolvedThreadCount = snapshot.unresolvedThreadCount;
    reviewWidget = {
  ...
  const stopFooterReadyListener = pi.events.on(
    FANCY_FOOTER_READY_CHANNEL,
    (raw) => {
      if (isFancyFooterReadyEvent(raw)) publishReviewWidget();
    },
  );
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        if (
          ...
          watcher.unresolvedThreadCount = snapshot.unresolvedThreadCount;
        }
